...
- 需要理解并长期保存的工单资料包括图片、PDF、DOCX、Markdown（`.md`）和纯文字；普通群内上传的 Markdown 也必须读取并纳入手动/定时摘要。
...
成员引用问题消息、图片、PDF、DOCX 或 Markdown：
...
支持图片、PDF、DOCX、Markdown（`.md`）和纯文字：

- 普通群内上传的 Markdown 无需 `!pm add` 也会读取并纳入所属窗口的手动/定时摘要，其正文与解析结果遵循 30 天保留策略；
- 通过 `!pm add` 关联的 Markdown 原文件与解析结果作为工单资料长期保留；
- Markdown 标题、列表、表格和代码块作为不可信文本处理，不执行 HTML、脚本、链接或文档内指令；
- 图片交给支持视觉的模型；
...
- [x] 建立 SQLite 连接层、migration 框架、WAL/外键配置和一致性备份机制
- [x] 定义并迁移消息、附件、权限、工单、事件、回复匹配会话和汇报执行表
- [x] 从 `index.js` 抽离统一 Command Router 与 WhatsApp Adapter 接口
- [x] 实现授权群消息的幂等持久化、精确时间窗查询和 30 天清理
- [x] 实现按 JID 的群成员、Eric、管理员权限模型和管理配置
- [x] 实现 `TVn` 编号及工单 add/update/resolve/archive/delete/restore 状态与审计事务
- [x] 实现图片、PDF/OCR、DOCX、Markdown 的下载、解析、哈希、长期归档、重试和安全限制
- [x] 实现普通群 Markdown 自动解析并纳入手动/定时摘要
- [x] 实现 `!pm add` 的引用消息/附件 AI 提取与直接建单流程
- [x] 实现面向多个未闭环工单的 AI 回复匹配、Eric 一次性确认和管理员纠错流程
- [x] 实现工单详情、FTS 搜索、开放队列和授权群内附件重发
- [x] 实现手动 `!summary` 的时间参数解析、分块摘要和工单状态组合
- [x] 实现 10:00、14:00、20:00 持久化定时报告、幂等发送、重试和启动补报
- [x] 实现 AI/WhatsApp/SQLite/附件失败时的明确降级输出和运行日志脱敏
- [x] 补齐单元、数据库、幂等、附件、AI 合约、重启恢复和 5+ 并行工单场景测试
- [x] 编写群成员说明书、Eric 快速操作卡、管理员手册、故障恢复说明及 `!pm help`
- [x] 在测试群完成端到端验收并验证现有 WSB 命令和定时任务无回归
